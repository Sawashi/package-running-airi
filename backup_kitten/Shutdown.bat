...
:: Write shutdown PS1 using echo (path passed via env var, no backslash escaping needed)
set PS_SCRIPT=%TEMP%\kitten_shutdown.ps1
echo Set-Location $env:KITTEN_TTS_DIR > "%PS_SCRIPT%"
echo Write-Host "Bringing down kitten-tts..." -ForegroundColor Cyan >> "%PS_SCRIPT%"
echo docker compose down >> "%PS_SCRIPT%"
echo Write-Host "Shutting down Docker Desktop..." -ForegroundColor Yellow >> "%PS_SCRIPT%"
echo Stop-Process -Name "Docker Desktop" -Force -ErrorAction SilentlyContinue >> "%PS_SCRIPT%"
