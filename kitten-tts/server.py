...
async def custom_tts_endpoint(
    ...
    # Clean input: strip timestamps and special characters before synthesis
    cleaned_input = utils.clean_text_for_tts(request.text)
    ...
    logger.debug(f"Input text (first 100 chars): '{cleaned_input[:100]}...'")
    ...
    if request.split_text and len(cleaned_input) > (
        ...
        text_chunks = utils.chunk_text_by_sentences(cleaned_input, chunk_size_to_use)
        perf_monitor.record(f"Text split into {len(text_chunks)} chunks")
    else:
        text_chunks = [cleaned_input]
        logger.info(
...
async def openai_speech_endpoint(request: OpenAISpeechRequest):
    ...
    try:
        # Clean input: strip timestamps and special characters before synthesis
        cleaned_text = utils.clean_text_for_tts(request.input_)
