...
def clean_text_for_tts(text: str) -> str:
    """
    Cleans input text for TTS synthesis by removing timestamps and special characters.

    - Removes [YYYY-MM-DD HH:MM] timestamps anywhere in the text
    - Strips all special characters except letters, digits, spaces, commas, and periods
    - Normalizes whitespace

    Args:
        text: Raw input text that may contain timestamps, emoji, quotes, etc.
    ...
    # Remove [YYYY-MM-DD HH:MM] timestamps (anywhere in text)
    text = re.sub(r'\[\d{4}-\d{2}-\d{2} \d{2}:\d{2}\]', '', text)

    # Strip special chars, keep only letters, digits, spaces, commas, periods
    text = re.sub(r'[^a-zA-Z0-9\s,.]', '', text)

    # Collapse multiple spaces (but preserve newlines as single space)
    text = re.sub(r'[^\S\n]+', ' ', text)
